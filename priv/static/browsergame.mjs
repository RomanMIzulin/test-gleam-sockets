        ...
        if ($1 === "") {
          ...
          let path_segments$1 = (() => {
            ...
            if ($2) {
              ...
              return append2(_pipe$1, split3(relative.path, "/"));
            }
          })();
          let path = (() => {
...
// build/dev/javascript/browsergame/ffi.mjs
...
// build/dev/javascript/browsergame/websockets.mjs
...
// build/dev/javascript/browsergame/browsergame.mjs
function rectangle() {
  let figure = canvas(
    toList([
      attribute("fillStyle", "green"),
      width(100),
      height(100),
      style(toList([["background", "green"]]))
    ])
  );
  return figure;
}
function smt() {
  let s = init2(
    "ws:localhost:1234/",
    (event) => {
      return println(inspect2(event));
    }
  );
  return s;
}
function main() {
  println("Hello from browsergame!");
  let app = element2(
    div(toList([]), toList([text("hello kek"), rectangle()]))
  );
  let s = smt();
